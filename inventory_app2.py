...
def plot_relative_performance(merged_df, custom_title=None,
    ...
    try:
        ...
        for policy in unique_policies:
             try: # Simplified legend handle finding
                idx = [i for i, l in enumerate(labels) if str(l) == policy or (isinstance(l, tuple) and policy in l)][0]
                if policy not in policy_labels_for_legend: policy_handles.append(handles[idx]); policy_labels_for_legend.append(policy)
             except Exception: pass
        # Apply fontsize to legend text and title
        ...
        if policy_handles: ax.legend(handles=policy_handles, labels=policy_labels_for_legend, title='Policy', bbox_to_anchor=(1.03, 0.55), loc='upper left',
                                      fontsize=legend_fontsize, title_fontsize=legend_fontsize) # Apply legend size
        else: pass
...
def plot_absolute_performance(input_df, custom_title=None,
    ...
    try:
        ...
        axes[0].grid(True); axes[0].legend(title='Policy', fontsize=legend_fontsize, title_fontsize=legend_fontsize); axes[0].set_ylim(bottom=0)
        ...
        axes[1].grid(True); axes[1].legend(title='Policy', fontsize=legend_fontsize, title_fontsize=legend_fontsize)
        ...
        fig.suptitle(plot_title, fontsize=title_axis_fontsize + 2, y=1.02) # Make title slightly larger
